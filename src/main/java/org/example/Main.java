...
//        sendNoticeTele();
        ...
        Service service2 = new Service("service 2", Category.CAT2, "5619763598", System.currentTimeMillis(), System.currentTimeMillis());
        service1.setToken(UUID.randomUUID().toString());
        service2.setToken(UUID.randomUUID().toString());
        User user = userRepository.findUserByIdTelegram("6784049826");
        service1.setOwner(user.getIdTelegram());
        service2.setOwner(user.getIdTelegram());
        user.getServices().add(service1);
        user.getServices().add(service2);
